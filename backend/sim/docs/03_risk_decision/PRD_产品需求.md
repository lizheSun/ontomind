    ...
    风险决策系统提供 **在申请、放款、贷中等关键节点执行规则+模型决策，返回批准/拒绝/复审** 的完整产品能力。
...
| policy_ref | 详见 schema.sql |
    ...
    ```mermaid
    graph LR
        A[进件 loan_intake, 客户中心 CIF] --> B[风险决策]
        B --> C[信贷核心 credit_core, 催收 collection]
        B --> D[(MySQL sim_risk_decision)]
    ```
