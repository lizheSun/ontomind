    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**营销系统**来支撑核心业务能力。该系统的核心职责是**渠道、活动、投放成本、归因、优惠码的管理**。
    ...
    | 上游系统 (外部广告平台) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (进件 loan_intake, 客户中心 CIF, 数仓) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `user_promo_use`
    ...
    详见 `../../systems/08_marketing/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
