    # 资金/合作方 (Funding & Partner) — BRD 业务需求文档

    - **系统代号**：Funding & Partner
    - **文档版本**：v1.0
    - **发布日期**：2024-11-30
    - **系统位置**：中台
    - **数据库**：`sim_funding`
    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**资金/合作方系统**来支撑核心业务能力。该系统的核心职责是**管理资金合作方、协议、每笔借据的资金拆分与分润记录**。
    ...
    | 上游系统 (信贷核心 credit_core) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (财务 finance) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `guarantee_record`
    ...
    详见 `../../systems/06_funding/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
