    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**进件受理系统**来支撑核心业务能力。该系统的核心职责是**承接客户借款申请，收集必需资料，触发风控决策**。
    ...
    | 上游系统 (APP/H5, 客户中心 CIF) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (风险决策 risk_decision, 信贷核心 credit_core) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `intake_channel_ref`
    ...
    详见 `../../systems/02_loan_intake/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
    ...
    - **上游**：APP/H5, 客户中心 CIF
    - **下游**：风险决策 risk_decision, 信贷核心 credit_core
...
作为公司**核心业务系统之一**，进件受理的成败直接影响以下 KPI：

1. **业务连续性**：作为中台系统，一旦出现故障将阻塞主流程
