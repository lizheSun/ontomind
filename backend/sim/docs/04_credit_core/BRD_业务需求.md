    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**信贷核心系统**来支撑核心业务能力。该系统的核心职责是**承载授信额度、支用（放款）、还款计划、还款流水、逾期状态的全生命周期**。
    ...
    | 上游系统 (风控决策 risk_decision, 进件 loan_intake) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (催收 collection, 财务 finance, 资金 funding) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `loan_status_log`
    ...
    详见 `../../systems/04_credit_core/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
    ...
    - **上游**：风控决策 risk_decision, 进件 loan_intake
    - **下游**：催收 collection, 财务 finance, 资金 funding
...
作为公司**核心业务系统之一**，信贷核心的成败直接影响以下 KPI：

1. **业务连续性**：作为业务中枢系统，一旦出现故障将阻塞主流程
