    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**客服系统**来支撑核心业务能力。该系统的核心职责是**客户工单、通话记录、投诉的统一处理**。
    ...
    | 上游系统 (APP, 电话呼叫中心, 微信) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (合规 legal, 运营) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `ticket_action`
    ...
    详见 `../../systems/10_csm/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
