    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**催收管理系统**来支撑核心业务能力。该系统的核心职责是**接收逾期借据，编排催收案件，跟踪催收动作与还款承诺**。
    ...
    | 下游系统 (客服 csm, 财务 finance) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `collector`
    ...
    详见 `../../systems/05_collection/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
