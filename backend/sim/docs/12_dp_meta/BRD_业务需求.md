    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**数据平台元数据系统**来支撑核心业务能力。该系统的核心职责是**维护数仓表元数据、ETL 任务、数据血缘**。
    ...
    | 上游系统 (数仓开发人员) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (数据部门内部使用) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `data_lineage`
    ...
    详见 `../../systems/12_dp_meta/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
