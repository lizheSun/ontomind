    ...
    信优消费金融公司在业务快速发展过程中，需要一套专门的**埋点收集系统**来支撑核心业务能力。该系统的核心职责是**APP/H5 埋点接入，页面浏览、事件、点击流三个粒度**。
    ...
    | 上游系统 (APP, H5, 微信小程序) | 提交业务请求，获取处理结果 | API 集成 |
    | 下游系统 (数仓 dw, 营销 marketing) | 消费该系统产生的业务事件 | 事件订阅 |
...
- `click_stream`
    ...
    详见 `../../systems/09_events/schema.sql`。表结构设计遵循：
    - 主键使用递增或雪花 ID
    - 状态字段带索引
