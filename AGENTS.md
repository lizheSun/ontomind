...
主要能力：
1. **AIDE** — iframe 嵌入 opencode Web UI（`/aide`）
2. **用户管理** — `/users`
3. **DataOps** — 数据仓库、智能数开、Wiki 知识库、元数据标注、本体建模
4. **AgentOps / Compute / Skill 平台** — Agent/Skill 设计与容器发布、算力节点

后端路由域（`/api/v1/`）：`auth` / `users` / `opencode` / `compute` / `agent-factory` / `skill-platform` / `dataops` / `wiki` / `metadata` / `ontology`。  
ORM **46 张表**（权威清单见 `app/db/models/__init__.py`）。`pytest` 当前 **88 passed**。
...
- 主题：**Apple Design**（SF Pro + `#0071e3` + `#F5F5F7`）
